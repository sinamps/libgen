...
def torch_cublas_time_us_parity(
    ...
    # - If c_major == "n": C_torch came from (L,M,N).permute(1,2,0) => (M,N,L)
    #   so the inverse view is (L,M,N).
    # - If c_major == "m": C_torch came from (L,N,M).permute(2,1,0) => (M,N,L)
    #   so the inverse view is (L,N,M).
    ...
    print(f"\nsinamps cublas: {A_rm.shape, A_rm.stride()}")
